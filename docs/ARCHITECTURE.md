...
**Tier 3 — scoped pixel comparison / goldens (tagged nodes only).** Deterministic pixel comparison, decided design: stabilized capture → widget-key masks applied to both images → vendored pixelmatch algorithm (Dart port of mapbox/pixelmatch, MIT: YIQ perceptual color distance + anti-aliasing detection that excuses edge pixels whose difference is explained by neighbors) → dual thresholds: per-pixel YIQ tolerance (default 0.1) and overall diff ratio over unmasked pixels (default 0.001), both overridable per node, defaults recalibrated from the first weeks of real skeleton runs. Output per comparison: ratio, verdict, red diff-overlay PNG to the run store. No AI in the verdict, ever — triage interprets downstream. Capture scope is *derived*, not hand-chosen: the recorder diffs the tree before/after the inbound edge and crops to the subtree the node added — route push → full screen; dialog/sheet/snackbar → crop to the overlay widget (via that subtree's RenderRepaintBoundary); in-place region change → that region or nothing (tier 1 already covers it). A small curated set of composition-critical screens may override to full_screen. Masks are widget-key based (resolved to rects at capture time) so they survive layout shifts. Baselines are keyed by (node, device, locale, theme) and content-addressed. Implementation lives in `applens_runner/lib/src/visual/` as four units: `pixelmatch.dart` (pure algorithm, validated against the upstream JS test fixtures — port the tests first, the algorithm second; watch the RGBA byte-format trap between `ui.Image.toByteData(rawStraightRgba)` and the algorithm's expected layout), `comparator.dart` (masks, thresholds, verdict), `capture.dart` (stabilization, scoped capture, mask resolution), `golden_compat.dart` (see applens_compare, §3).
...
**Tier 1 (applens_sdk in the app, opt-in).** Adds: `TestClock` (injectable clock — mandatory in practice for any app that renders time); a state introspection channel implemented as a dart:developer service extension (the app registers cheap state probes: `applens.flags` returns {cart_count: 3, journey_started: true}), enabling precise flag-based identity and state assertions; seed hooks (named reset-to-known-state entry points the runner can invoke between paths). All of it compiled out of release builds.
